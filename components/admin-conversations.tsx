...
export function AdminConversations() {
  ...
  const columns: Column<IncomingMessage>[] = [
    {
      key: "sender",
      header: "Sender",
      accessor: (m) => m.senderName || m.senderContact,
      cell: (m) => (
        <div>
          <div className="font-medium">{m.senderName || m.senderContact}</div>
          <div className="text-sm text-muted-foreground">{m.senderContact}</div>
        </div>
      ),
      sortable: true,
    },
    {
      key: "message",
      header: "Message",
      accessor: (m) => m.message,
      cell: (m) => <div className="text-sm">{m.message}</div>,
    },
    {
      key: "source",
      header: "Source",
    ...
    },
    {
      ...
      cell: (m) => formatDateTimeUTC(m.receivedAt),
      sortable: true,
    },
    {
      key: "processed",
      header: "Processed",
      accessor: (m) => m.processed,
      cell: (m) => (m.processed ? "Yes" : "No"),
    },
    {
      key: "actions",
      header: "",
      cell: (m) => (
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={async () => {
              await MockAPI.markMessageProcessed(m.id, !m.processed);
              await load();
            }}
          >
            {m.processed ? "Unmark" : "Mark"}
          </Button>
          <Button
            variant="ghost"
            onClick={() => {
  ...
  const parseMessageToBooking = (msg: IncomingMessage) => {
    ...
    const parsed: Partial<Booking> = {
      ...
      email: "",
      flightNumber: flightMatch
        ? flightMatch[0].replace(/\s+/g, " ").toUpperCase()
        : "",
      ...
      status: "new",
      source:
        msg.source === "whatsapp" || msg.source === "email"
          ? msg.source
          : "manual",
    };
  ...
  const createBookingFromMessage = async () => {
    ...
    try {
      ...
      await MockAPI.createActivityLog({
        ...
        action: "Booking created from message",
        user: "Agent",
        details: `Created from message ${selected.id}`,
      });
      ...
      toast.showToast({
        ...
        description: `Created ${booking.passengerName}`,
        type: "success",
      });
    } catch (err) {
      console.error(err);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Messages from WhatsApp, Email, Calls - convert to bookings
          </p>
        </div>
      ...
      {selected && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Message from {selected.senderName || selected.senderContact}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Input
                ...
                value={parsed.passengerName || ""}
                onChange={(e) =>
                  setParsed({ ...parsed, passengerName: e.target.value })
                }
              />
              <Input
                placeholder="Phone"
                value={parsed.phone || ""}
                onChange={(e) =>
                  setParsed({ ...parsed, phone: e.target.value })
                }
              />
              <Input
                ...
                onChange={(e) =>
                  setParsed({ ...parsed, flightNumber: e.target.value })
                }
              />
              <DateTimePicker
                ...
                onChange={(d, t) =>
                  setParsed({ ...parsed, date: d || "", time: t || "" })
                }
              />
              <Input
                placeholder="Special requests"
                value={parsed.specialRequests || ""}
                onChange={(e) =>
                  setParsed({ ...parsed, specialRequests: e.target.value })
                }
              />
            </div>
            <div className="flex gap-2 mt-4">
              <Button onClick={createBookingFromMessage} disabled={creating}>
