...
export function AdminTasks() {
  ...
  const loadAgents = async () => {
    try {
      setAgents(await MockAPI.getAgents());
    } catch (err) {
      console.error(err);
    }
  };
  const loadBookings = async () => {
    try {
      setBookings(await MockAPI.getBookings("all", 200));
    } catch (err) {
      console.error(err);
    }
  };
          ...
          <CardContent>
            ...
            >
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, relatedBookingId: v })}
              >
                <SelectTrigger className="w-full">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  ...
                  {bookings.map((b) => (
                    <SelectItem value={b.id} key={b.id}>
                      {b.passengerName} • {b.flightNumber}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <CardContent className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  Assigned to:{" "}
                  {agents.find((a) => a.id === t.assignedTo)?.name ||
                    "Unassigned"}
                </div>
                {t.relatedBookingId && (
                  <div className="text-sm">
                    Related:{" "}
                    {
                      bookings.find((b) => b.id === t.relatedBookingId)
                        ?.passengerName
                    }
                  </div>
                )}
              ...
              <div className="flex items-center gap-2">
                <Select
                  value={t.status}
                  onValueChange={(v) =>
                    updateStatus(t.id, v as TaskItem["status"])
                  }
                >
