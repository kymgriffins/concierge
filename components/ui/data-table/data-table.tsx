...
export function DataTable<T extends Record<string, any>>(
  ...
  const [pageSize, setPageSize] = React.useState(
    isMobile ? Math.min(10, defaultPageSize) : defaultPageSize,
  );
  ...
  const filteredData = React.useMemo(() => {
    ...
    Object.entries(filters).forEach(([key, filterValue]) => {
      if (filterValue) {
        result = result.filter((row) => {
          ...
          if (!col) return true;
          const value = col.accessor ? col.accessor(row) : row[key as keyof T];
          return String(value)
            .toLowerCase()
    ...
    if (sort.key && sort.dir) {
      ...
      if (col) {
        const accessor = col.accessor || ((r: T) => r[sort.key as keyof T]);
        result.sort((a, b) => {
          const av = accessor(a);
          const bv = accessor(b);
