...
export function ServiceLifecycleManager({
  ...
  onStatusChange,
  compact = false,
}: ServiceLifecycleManagerProps) {
  const [lifecycleStatus, setLifecycleStatus] =
    useState<ServiceLifecycleStatus | null>(null);
  ...
  const getStatusConfig = (status: Booking["status"]): StatusConfig => {
    const configs: Record<Booking["status"], StatusConfig> = {
      new: {
        ...
        color: "default",
        icon: AlertCircle,
        description: "Booking created, awaiting contact",
      },
      contacted: {
        label: "Contacted",
        color: "secondary",
        icon: UserCheck,
        description: "Passenger has been contacted",
      },
      confirmed: {
        label: "Confirmed",
        color: "outline",
    ...
    return (
      <div className="flex items-center gap-3 p-3 border rounded-lg">
        ...
        <div className="flex gap-1">
          {lifecycleStatus.nextPossibleStatuses.map(
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Manage the {booking.serviceId} service lifecycle for{" "}
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center gap-4 p-4 border rounded-lg">
          ...
          <div className="flex-1">
            ...
            {lifecycleStatus.timeUntilAutoTransition &&
              lifecycleStatus.timeUntilAutoTransition > 0 && (
                <p className="text-xs text-orange-600 mt-1">
                  Auto-transition in{" "}
                  {Math.floor(lifecycleStatus.timeUntilAutoTransition / 60)}h{" "}
                  {lifecycleStatus.timeUntilAutoTransition % 60}m
                </p>
              )}
          </div>
          {lifecycleStatus.canAutoTransition && (
              ...
              {[
                ...
                const isCompleted =
                  [
                    ...
                    "completed",
                    "pending_review",
                  ].indexOf(lifecycleStatus.currentStatus) >= index;
