...
const SERVICE_KEYWORDS = {
  'meet_greet': ['meet', 'greet', 'meet and greet', 'meeting', 'greeting', 'pickup'],
  'fast_track': ['fast track', 'priority', 'express', 'quick security', 'fast security'],
  'lounge': ['lounge', 'vip lounge', 'business lounge', 'executive lounge'],
  'porter': ['porter', 'baggage', 'luggage', 'carry', 'porter service'],
  'chauffeur': ['chauffeur', 'driver', 'car', 'transport', 'limo', 'ground transport'],
  'vip_package': ['vip package', 'complete', 'full service', 'premium', 'all services']
};
...
export class MessageParser {
  ...
  static parse(message: string): MessageParseResult {
    try {
      ...
      // Parse booking data
      const bookingData = this.extractBookingData(cleanMessage, extractedEntities);

      // Calculate confidence scores
      const confidence = this.calculateConfidence(bookingData, extractedEntities);

      // Determine if we have enough information for a booking
      const hasMinimumData = bookingData.flightNumber && (bookingData.date || bookingData.time);
  ...
  private static extractEntities(message: string): ParsedBookingData['extractedEntities'] {
    return {
  ...
  private static extractNames(message: string): string[] {
    ...
    const commonWords = ['the', 'and', 'for', 'with', 'from', 'this', 'that', 'book', 'flight', 'airport', 'please', 'hi', 'hello'];
  ...
  private static extractServices(message: string, activeServices?: string[]): string[] {
  ...
  private static extractBookingData(message: string, entities: ParsedBookingData['extractedEntities']): Omit<ParsedBookingData, 'confidence'> {
    // Extract flight info
    let flightNumber: string | undefined;
    let airline: string | undefined;

    if (entities.flightNumbers.length > 0) {
      const flightMatch = entities.flightNumbers[0].match(/([A-Z]{2,3})\s*(\d{3,4})/);
      if (flightMatch) {
    ...
    const passengerName = entities.names.length > 0 ? entities.names[0] : undefined;
    ...
    const passengerMatch = message.match(/(\d+)\s*(?:passengers?|people|travelers?|guests?)/i);
    if (passengerMatch) {
  ...
  private static calculateConfidence(
    ...
    const scores = {
      ...
      flight: (entities.flightNumbers.length > 0 ? 0.95 : 0) * (data.airline ? 1.0 : 0.8),
      dateTime: Math.min((entities.dates.length + entities.times.length) * 0.5, 1.0),
  ...
  private static generateSuggestions(entities: ParsedBookingData['extractedEntities']): string[] {
    ...
    if (entities.dates.length === 0 && entities.times.length === 0) {
      suggestions.push('Include date and/or time (e.g., "tomorrow at 2:30 PM" or "2025-01-15")');
    }
